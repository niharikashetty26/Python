...
def add_book(customer_id):
    try:
        ...
        if result:
            ...
            """
            execute_and_commit(order_query, (customer_id, total_price, [book_id], [quantity_to_order]),
                               transactional=True)
...
def view_orders(customer_id):
    try:
        order_query = """
        SELECT order_id, order_date, total, books, quantity 
        FROM Orders 
        WHERE customer_id = %s;
        """
        ...
        if orders:
            print("Orders:")
            for order in orders:
                order_id, order_date, total, books, quantities = order
                print(f"Order ID: {order_id}, Order Date: {order_date}, Total Price: {total}")
